...
左侧core agent管理系统整体流程和每个module的project agent。绿色的流向线代表当前core agent正在向不同的module派发需求。他们并行处理。
![agent_frontend.png](./pic/agent_frontend.png)

左下角的聊天窗是其中一个入口，可以把需求持续放进core的队列。  
画面中心的module节点里的聊天泡泡和对话框，表示当前module的agent被decision block了，需要用户主动输入ai需要的信息去unblock。  
![input_decision.png](./pic/input_decision.png)
